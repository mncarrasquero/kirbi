...
-- Create comprehensive policies for materials
CREATE POLICY "Enable read access for authenticated users" ON materials
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "Enable insert access for authenticated users" ON materials
  FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Enable update access for authenticated users" ON materials
  FOR UPDATE TO authenticated USING (auth.uid() IS NOT NULL);

CREATE POLICY "Enable delete access for authenticated users" ON materials
...
-- Create comprehensive policies for inventory_entries
CREATE POLICY "Enable read access for authenticated users" ON inventory_entries
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "Enable insert access for authenticated users" ON inventory_entries
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Enable update access for own entries" ON inventory_entries
  FOR UPDATE TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "Enable delete access for own entries" ON inventory_entries
